# GM_EXPENSES_RUNTIME_REHEARSAL_FINAL_12 - post-migration verification of the disposable copy (V63) against read-only Shared DEV (V43).
...
$name = (Get-Content "$scratch\f12-container-name.txt").Trim()
...
function Copy-Sql([string]$sql) {
    $out = ("SET SESSION TRANSACTION READ ONLY;`nSTART TRANSACTION READ ONLY;`n" + $sql + "`nROLLBACK;") |
        docker exec -i $name sh -c 'MYSQL_PWD=$MYSQL_ROOT_PASSWORD exec mysql -uroot --batch --skip-column-names gypport_rehearsal_final12' 2>$null
...
function Dev-Sql([string]$sql) {
    $out = ("SET SESSION TRANSACTION READ ONLY;`nSTART TRANSACTION READ ONLY;`n" + $sql + "`nROLLBACK;") |
        docker exec -i -e MYSQL_PWD gypport-mysql-dev mysql "-u$devUser" --batch --skip-column-names core_business_dev 2>$null
    if ($LASTEXITCODE -ne 0) { throw 'Shared DEV read-only query failed.' }
    return $out
}
try {
    "## flyway"
    Copy-Sql "SELECT CONCAT('max=', MAX(CAST(version AS UNSIGNED)), ' rows=', COUNT(*), ' failed=', SUM(success = 0), ' applied_44_63=', SUM(CAST(version AS UNSIGNED) BETWEEN 44 AND 63 AND success = 1), ' beyond_63=', SUM(CAST(version AS UNSIGNED) > 63)) FROM flyway_schema_history WHERE version IS NOT NULL;"

    "## row counts: every table present before (copy = Shared DEV V43) vs after (copy V63)"
    $before = @{}; foreach ($line in (Get-Content "$scratch\f12-copy-before-counts.txt")) { if ($line) { $t, $n = $line.Split("`t"); $before[$t] = [long]$n } }
    ...
    "tables_with_changed_counts=[$(($changed | Sort-Object) -join '; ')]"
    "tables_added_by_migrations=[$(($added | Sort-Object) -join ', ')]"
    $lost = @($changed | Where-Object { $_ -notmatch '^flyway_schema_history ' } | Where-Object { $parts = $_ -split ' |->'; [long]$parts[2] -lt [long]$parts[1] })
    "business_tables_with_fewer_rows=[$($lost -join '; ')]"
    ...
    "## triggers after migration vs before"
    $triggersBefore = @(Get-Content "$scratch\f12-copy-before-triggers.txt" | Where-Object { $_ })
    $triggersAfter = @(Copy-Sql "SELECT TRIGGER_NAME, EVENT_OBJECT_TABLE, ACTION_TIMING, EVENT_MANIPULATION, DEFINER, SHA2(ACTION_STATEMENT, 256) FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA = DATABASE() ORDER BY TRIGGER_NAME;")
    $removedTriggers = @($triggersBefore | Where-Object { $triggersAfter -notcontains $_ } | ForEach-Object { ($_ -split "`t")[0] })
    $newTriggers = @($triggersAfter | Where-Object { $triggersBefore -notcontains $_ } | ForEach-Object { ($_ -split "`t")[0] })
    "triggers_before=$($triggersBefore.Count) triggers_after=$($triggersAfter.Count) changed_or_removed=[$($removedTriggers -join ', ')] new_or_changed=[$($newTriggers -join ', ')]"
    Copy-Sql "SELECT CONCAT('routines_left_by_migrations=', COUNT(*)) FROM information_schema.ROUTINES WHERE ROUTINE_SCHEMA = DATABASE();"
...
"@
    ...
    Dev-Sql "SELECT CONCAT('shared_dev_flyway_max=', MAX(CAST(version AS UNSIGNED)), ' history_rows=', COUNT(*), ' failed=', SUM(success = 0)) FROM flyway_schema_history WHERE version IS NOT NULL;"
}
finally {
