...
$api = 'http://127.0.0.1:8080'; $mailpit = 'http://127.0.0.1:8025'
$scratch = 'C:\Users\elbur\AppData\Local\Temp\claude\D--NZXTG7-GYPPORT-GYPPORT-ERP\6bd391c2-9dd7-4c98-8f41-546e5a9baea1\scratchpad\s18\smoke'
...
function Login([string]$tag) {
    $s = New-Session
    $email = "deploy-smoke-s18$tag-$([guid]::NewGuid().ToString('N').Substring(0,8))@example.test"; $password = 'Ds-Smoke-' + [guid]::NewGuid().ToString('N')
    $null = Send $s 'POST' '/auth/register' @{ firstName = 'Runtime'; firstSurname = "SmokeS18$tag"; email = $email; password = $password }
    $code = $null
    ...
    $s.Email = $email
    $s.Responsible = (Send $s 'POST' '/api/expense-persons' @{ operationId = (Op); displayName = "S18 Responsable $tag"; primaryEmail = "s18-$tag-$([guid]::NewGuid().ToString('N').Substring(0,8))@example.test" }).Json.reference
    return $s
}
...
function Add-Advance($s, [string]$caseId, [string]$amount) {
    return Send $s 'POST' "/api/expense-cases/$caseId/advances" @{ operationId = (Op); activityType = 'VIAJE'; activityDescription = "S18 tramo $amount"; amount = $amount; currency = 'USD'; deliveryMethodCode = 'EFECTIVO'; renditionDays = 7 }
}
...
function Movement($s, [string]$caseId, [string]$kind, [string]$amount) {
    return Send $s 'POST' "/api/expense-cases/$caseId/rendition/$kind" @{ operationId = (Op); expectedRenditionVersion = (Rendition $s $caseId).version; amount = $amount; currency = 'USD'; reason = 'S18 deployment smoke' }
}
...
$c9 = New-Case $a 'S18 Construcción'
...
$c10 = New-Case $a 'S18 Tramo adicional'
...
$c11 = New-Case $a 'S18 Devolución y reembolso'
...
$c13 = New-Case $a 'S18 Flujo de gastos y bloqueos'
...
$c15 = New-Case $a 'S18 Viaje Loja (ensayo final)'
...
$list | ConvertTo-Json -Depth 12 | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $scratch 's18-tenant-a-case-list.json')

# ---- §17 per-Case directions, then reports (+30 and -20 never net)
$cPlus = New-Case $a 'S18 Por justificar'
$null = Delivered-Advance $a $cPlus '100'; $null = Approved $a $cPlus '70'; $null = Registered $a $cPlus '15'
$cMinus = New-Case $a 'S18 Por reembolsar'
...
$cZero = New-Case $z 'S18 Sin anticipo entregado'
...
$zPeriod.Text | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $scratch 's18-zero-tenant-period-summary.json')
$zRelated | ConvertTo-Json -Depth 5 | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $scratch 's18-zero-tenant-related-funding.json')
$zDetail.Text | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $scratch 's18-zero-tenant-case-detail.json')
...
$ids | ConvertTo-Json | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $scratch 's18-smoke-ids.json')
$results.Add("S18_SERVER_5XX=$server5xx [$($server5xxCalls -join '; ')]")
$results.Add("S18_DEPLOY_CHECKS=$($results.Count - 1) FAILURES=$failures")
$results | Set-Content -Encoding UTF8 -LiteralPath (Join-Path $scratch 's18-runtime-smoke-results.txt')
$results
