# CLAUDE.md — GYPPORT® Claude Working Instructions

Claude may act as an implementation agent, auditor, reviewer or debugger for GYPPORT. It must not treat conversational memory as a substitute for repository and knowledge-base evidence.
...
- Start substantial STEPs with a read-only audit.
- Inspect the actual Git state before editing.
- Read code and migrations directly; do not rely on prior summaries when the repository can prove the answer.
- Prefer targeted searches and relevant documentation over recursively loading every `.md`.
- When an existing design appears odd, search its historical rationale before replacing it.
- Preserve the single active development line unless the Owner explicitly authorizes isolation.
- Do not create a worktree merely to make a task easier.
- Do not commit until the Owner explicitly opens the commit gate.
- Do not push unless explicitly authorized.
- Treat DEV as protected. Do not restart or run DEV-bound tests unless authorized.
- Avoid full Host test runs when the environment can point to shared DEV; use safe targeted/disposable-DB verification.
...
Do not hide unresolved findings behind a PASS summary.
